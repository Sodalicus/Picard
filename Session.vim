...
imap <F7> :wa
map  :NERDTreeToggle
nnoremap   za " Enable folding with the spacebar
...
$argadd main.py
$argadd sensors.py
$argadd templates/index.html
$argadd static/style.css
...
wincmd w
argglobal
if bufexists("Picard.ino") | buffer Picard.ino | else | edit Picard.ino | endif
let s:cpo_save=&cpo
set cpo&vim
imap <buffer> <F8> :w:exec '!arduino-cli compile --upload -p /dev/ttyUSB0 -b arduino:avr:nano:cpu=atmega328old' shellescape(@%, 1) 
map <buffer> <F8> :w:exec '!arduino-cli compile --upload -p /dev/ttyUSB0 -b arduino:avr:nano:cpu=atmega328old' shellescape(@%, 1)  
let &cpo=s:cpo_save
unlet s:cpo_save
setlocal keymap=
